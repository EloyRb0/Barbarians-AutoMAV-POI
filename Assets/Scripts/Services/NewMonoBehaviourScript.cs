...
public static class OllamaComparer
{
    ...
    /// <param name="verbose">When true, logs the request & response to the Console.</param>
        ...
        Action<MatchObj> onDone, Action<string> onError = null, bool verbose = true)
    {
        ...
        catch { onError?.Invoke("Failed to parse envelope"); yield break; }
        ...
        try { verdict = JsonUtility.FromJson<MatchObj>(content); }
            ...
            // Try to slice out JSON
            if (!string.IsNullOrEmpty(content))
            {
                int i = content.IndexOf('{'); int j = content.LastIndexOf('}');
                if (i >= 0 && j > i)
                    try { verdict = JsonUtility.FromJson<MatchObj>(content.Substring(i, j - i + 1)); } catch {}
            }
        }
